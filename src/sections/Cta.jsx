...
const CTA = () => {
    ...
    return (
        <section id="cta" className="relative overflow-hidden bg-orange-500 px-6 flex flex-col justify-center items-center min-h-100 md:min-h-110 mt-44">
            <div className="abssolute inset-0 w-full max-w-7xl mx-auto pointer-events-none">
                {floatingDishes.map((dish) => (
                    <img key={dish.src} src={dish.src} slt={dish.alt} className={`absolute size-20 md:size-28 lg:size-35 rounded-full object-cover pointer-events-auto transition-all duration-300 hover:scale-105 ${dish.className}`} />
                ))}
            </div>
            ...
            <div className="relative z-10 text-center max-w-2xl mx-auto">
                ...
                <Animated delay={0.2} className="flex items-center justify-center">
                    <a href="#booking-process" className="flex items-center gap-2.5 bg-white text-black pl-5 pr-2 py-2 rounded-full mt-5.5 transition group hover:scale-105 duration-300">
                        {t('cta.button')}
                        <span className="size-7 rounded-full bg-black text-white grid place-content-center">
                            <ArrowRight size={16} className="transition-transform group-hover:translate-x-1 duration-400"/>
                        </span>
                    </a>
                </Animated>
            </div>
